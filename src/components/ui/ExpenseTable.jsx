...
const ExpenseTable = () => {
  ...
  const handleCheckboxChnage = async (expenseId) => {
    const newStatus = !checkedItems[expenseId];
    try {
      const res = await axios.put(`https://expense-tracker-backend-m0fv.onrender.com/api/v1/expense/${expenseId}/done`, { done: newStatus }, {
        headers: {
      ...
      if (res.data.success) {
        ...
        //optionally update local state for expense id the entire object needs update
  ...
  const removeExpenseHandler = async (expenseId) => {
    try {
      const res = await axios.delete(`https://expense-tracker-backend-m0fv.onrender.com/api/v1/expense/remove/${expenseId}`)
      if (res.data.success) {
        toast.success(res.data.message);
          ...
          <TableRow key={expense._id}>
            ...
            <TableCell className="text-right">
              <div className="flex items-center justify-end gap-2">
                ...
                {/* <Button size="icon"className="rounded-full border text-red-600 border-red-600 hover:border-transparent" variant="outline"><Edit2 className="w-4 h-4" /></Button> */}
